  ...
  <div class="test-wrapper">
    ...
    <div v-for="(q, qIndex) in exercises" :key="qIndex" class="question-block">
      ...
      <button
        ...
        class="btn-small btn-remove-question"
      >
        Видалити питання
      </button>
      ...
      <div v-if="q.type === 'one' || q.type === 'many'" class="answers-block">
        <div v-for="(a, aIndex) in q.answers" :key="aIndex" class="answer-row">
          ...
          <button @click="removeAnswer(qIndex, aIndex)" class="btn-small">
            Видалити
          </button>
        </div>
        <button @click="addAnswer(qIndex)" class="btn-small btn-add">
          Додати варіант
        </button>
      </div>
      ...
      <div v-if="q.type === 'enter'" class="enter-block">
        <div
          v-for="(r, rIndex) in q.correctAnswers"
          :key="rIndex"
          class="enter-row"
        >
          <input
            v-model="q.correctAnswers[rIndex]"
            placeholder="Правильна відповідь"
            class="enter-input"
          />
          <button
            @click="removeCorrectAnswer(qIndex, rIndex)"
            class="btn-small"
          >
            Видалити
          </button>
        </div>
        <button @click="addCorrectAnswer(qIndex)" class="btn-small btn-add">
          Додати відповідь
        </button>
      </div>
      ...
      <div v-if="q.type === 'pair'" class="pairs-block">
        <div class="pair-columns">
          <div>
            <h6>Ліва колонка</h6>
            <div
              v-for="(left, lIndex) in q.pairs.left"
              :key="'left-' + qIndex + '-' + lIndex"
              class="pair-row"
            >
              <input
                v-if="!left.isImage"
              ...
              <select v-model="q.pairs.correctMap[lIndex]">
                <option
                  v-for="(right, rIndex) in q.pairs.right"
                  :key="rIndex"
                  :value="rIndex"
                >
                  {{ rIndex + 1 }}
              ...
              <button @click="removeLeft(qIndex, lIndex)" class="btn-small">
                Видалити лівий
              </button>
            </div>
            <button @click="addLeft(qIndex)" class="btn-small btn-add">
              Додати лівий
            </button>
          </div>
          <div>
            ...
            >
              ...
              <button @click="removeRight(qIndex, rIndex)" class="btn-small">
                Видалити правий
              </button>
            </div>
            <button @click="addRight(qIndex)" class="btn-small btn-add">
              Додати зайвий правий
            </button>
          </div>
        </div>
      </div>
    ...
    <div class="btn-down">
      ...
      <button @click="updateTest" class="btn-create-test">
        Зберегти зміни
      </button>
    </div>
    ...
    <div v-if="showModal" class="modal-overlay" @click="showModal = false">
      <div class="modal-content animated-modal" @click.stop>
        ...
        <div class="modal-el">
          <p>
            <strong>Код тесту:</strong>
            <span class="code-text">{{ testCode }}</span>
            <button @click="copyToClipboard(testCode)" class="btn-copy">
              📋
            </button>
          </p>
        </div>
        <div class="modal-el">
          ...
          <p>
            ...
            <button @click="copyToClipboard(testLink)" class="btn-copy">
              📋
            </button>
          </p>
        </div>
  ...
  methods: {
        ...
        this.exercises = test.exercises.map((q) => {
          ...
          return {
            ...
            answers: q.answers.map((a) => ({
              ...a,
              // Додаємо BACK_URL, якщо це зображення і шлях існує
              preview:
                a.isImage && a.imageUrl ? `${BACK_URL}${a.imageUrl}` : null,
            })),
            pairs:
              q.type === "pair" && q.pairs
                ? {
      ...
      try {
        ...
        const exercisesData = this.exercises.map((q) => {
          const answers = (q.answers || []).map((a, i) => ({
            ...
            correct:
              q.type === "one" ? i === q.correctAnswerIndex : !!a.correct,
            isImage: !!a.isImage,
          }));
          const pairs =
            q.type === "pair"
    ...
    addLeft(qIndex) {
      ...
      if (q.pairs.right.length < q.pairs.left.length)
        q.pairs.right.push({ text: "", isImage: false, file: null });
    },
    addRight(qIndex) {
    ...
    removeLeft(qIndex, lIndex) {
      ...
      q.pairs.left.forEach((_, index) => {
        newMap[index] = q.pairs.correctMap[index] ?? 0;
      });
