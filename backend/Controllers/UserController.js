...
export const register = async (req, res) => {
  try {
    ...
    sendVerificationCode(email, user._id, UserModel).catch((err) =>
...
export const verifyEmail = async (req, res) => {
  try {
    ...
    const isValid = bcrypt.compare(code, user.emailCodeHash);
    ...
    // Генеруємо токен
    const token = jwt.sign({ _id: doc._id }, process.env.TOKEN_KEY, {
      expiresIn: "30d",
    });
...
export const sendLink = async (req, res) => {
  try {
    ...
    sendLinkForgot(email, user._id, link, UserModel).catch((err) =>
      console.error("Email error:", err),
    );
...
export const resetPassword = async (req, res) => {
  try {
    ...
    const isValid = bcrypt.compare(token, user.emailCodeHash);
...
export const userProfile = async (req, res) => {
  try {
    ...
    const [user, rawTests] = await Promise.all([
      ...
      TestModel.find({ author: req.userId })
        .select("title slug exercises.question")
        .lean(),
    ]);
